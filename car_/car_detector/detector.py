...
datapath = "CarData/TrainImages/"# path to CarData TrainImages.
SAMPLES = 125 #400
...
def get_bow_extactor(extract,flann):
...
def bow_features(img,extractor_bow,detector):
	return (extractor_bow.compute(im,dgetector.detect(img)))
...
def car_detector():
	...
	extract_bow = BOWImgDescriptorExtractor(extract,flann)
	...
	for i in range(SAMPLES):
		...
		traindata.extend(bow_features(cv2.imread(path(nen,i),0),extract_bow,detect))
		trainlabels.append(-1)
